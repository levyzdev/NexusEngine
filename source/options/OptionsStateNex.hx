...
class OptionsStateNex extends MusicBeatState
{
	var options:Array<String> = [
		'Customization', 
		#if DISCORD_ALLOWED 'Discord Settings' #end
	];
	...
	function openSelectedSubstate(label:String) {
		switch(label) {
			...
			case 'Discord Settings':
                openSubState(new options.DiscordSubState()); 
		}
	}
	...
	override function create() {
		#if DISCORD_ALLOWED
		DiscordClient.changePresence("In Nexus Settings", null);
		#end
		...
		var Inst:FlxText = new FlxText(12, FlxG.height - 24, 0, "Press Arrow Left to switch settings menu", 12);
	...
	override function closeSubState()
   {
      ...
      if (subState is CustomizeSubState)
      {
          var custom = cast(subState, CustomizeSubState);
          if (custom.wasClosed)
          {
               	FlxG.sound.music.stop();
				FlxG.sound.playMusic(Paths.music(ClientPrefs.data.menuSong), 0);
				FlxG.sound.music.fadeIn(4, 0, 0.7);
				custom.wasClosed = false;
          }
      }  
