...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music(ClientPrefs.data.menuSong), 0);
			}
		}
		...
		#if DISCORD_ALLOWED
		// Update RPC
		DiscordClient.changePresence("In Intro", null);
		#end
		...
		function tweenLogoBl():Void {
			FlxTween.tween(logoBl, {angle: -5}, 1.2, { 
		...
		blackScreen = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, 0xFF1A0036);
		credGroup.add(blackScreen);

		credTextShit = new FlxText(0, 0, 0, "", 64);
		credTextShit.setFormat("comic.ttf", 64, FlxColor.WHITE, CENTER);
		...
		psychLogo = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('psych_logo'));
		add(psychLogo);
		psychLogo.visible = false;
		psychLogo.setGraphicSize(Std.int(psychLogo.width * 0.3));
		psychLogo.updateHitbox();
		psychLogo.screenCenter(X);
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
	{
		for (i in 0...textArray.length)
		{
			...
			money.setFormat("comic.ttf", 64, FlxColor.WHITE, CENTER);
	...
	function addMoreText(text:String, ?offset:Float = 0)
	{
		if(textGroup != null && credGroup != null) {
			var randomColor = FlxColor.fromRGB(FlxG.random.int(0, 255), FlxG.random.int(0, 255), FlxG.random.int(0, 255));
			var coolText:FlxText = new FlxText(0, 0, 0, text, 64);
			coolText.setFormat("comic.ttf", 64, randomColor, CENTER);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 4:
					#if NEXUS_WATERMARKS
					addMoreText('levyzdev', 60);
					#elseif PSYCH_WATERMARKS
					addMoreText('Shadow Mario', 'Riveren', 60);
					#else
					addMoreText('present');
					#end
				case 5:
				...
				case 6:
					#if NEXUS_WATERMARKS
					createCoolText(['This associated', 'a'], -40);
					#elseif PSYCH_WATERMARKS
